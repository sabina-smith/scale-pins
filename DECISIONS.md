...
- **Light brown is the default colour, for everything, from now on.** The palette is a ten-step brown scale defined as Tailwind theme tokens in `app/globals.css` (`--color-brown-50` … `900`); `brown-200` is the page base and the shader's base colour matches it. Any new surface, component, or state starts from this scale rather than a grey. Set by the design owner on 2026-09-03.
