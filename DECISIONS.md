...
- **Vercel + Neon, provisioned through the Vercel marketplace.** Next.js is Vercel's native runtime and the marketplace integration creates the database and injects `DATABASE_URL` in one step, so there is one account and one dashboard. Both free tiers cover an internal board. Railway would have been the single-vendor alternative; Fly would have needed a Dockerfile.
